...
async def get_channel(ctx, channel_name):
    try:
        channel = next(filter(lambda c: channel_name in c.name, ctx.guild.text_channels))
        return channel
...
def format_messages(messages, emoji_names=True, emphasize_names=True): 
    ...
    return "\n".join(map(lambda msg : 
        pre + get_nick(msg.author) + post + ": " + fix_emoji(msg.clean_content) + get_emoji_strings(msg, emoji_names),# + format_embeds(msg), 
        messages
    ))
...
async def get_messages(channel, num_messages, time=None, carry_messages=[], iteration=0, from_user=None, other_filter=None, filter_doomer=True):
    if time == None:
    ...
    if ((num_messages - new_message_count) > 0 or new_message_count == 0) and iteration < 20:
        time = raw_messages[0].created_at
        new_result = await get_messages(channel, num_messages, time, filtered_messages, iteration + 1, from_user, other_filter)
        return new_result
    else:
...
def find_questions_and_answers(input_messages):
    ...
    for message in input_messages:
        is_question = bool(re.match("(\\?\\s)|(who)|(what)|(when)|(where)|(why)|(can (you|i|we|they))",message.clean_content.lower())) or message.clean_content.endswith("?")
        if preserve_next:
            qa.append(message)
            results.append(qa)
            preserve_next = False
